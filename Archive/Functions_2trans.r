...
dat.sim.reg = function(n,par,iseed,Zbin,Wbin){
  ...
  Y = pmin(T,C) # observed non-transformed time
...
LikF = function(par,Y,Delta,M){ 
  ...
  tot = (((1/sigma1)*dnorm(z1)*(1-pnorm(z2))*DtransY.T)^Delta)*((1/sigma2)*dnorm(z3)*(1-pnorm(z4))*DtransY.C)^(1-Delta) # likelihood
  p1 = pmax(tot,1e-100)   
  Logn = sum(log(p1)); 
  return(-Logn)
}
...
LikFG1 = function(par,Y,Delta,M){ 
  ...
  tot = (((1/sigma1)*dnorm(z1)*(1-pnorm(z2))*DtransY.T)^Delta)*((1/sigma2)*dnorm(z3)*(1-pnorm(z4))*DtransY.C)^(1-Delta) 
  p1 = pmax(tot,1e-100)   
  Logn = sum(log(p1)); 
...
LikI = function(par,Y,Delta,M){ 
  ...
  tot = (((1/sigma1)*dnorm(z1)*(1-pnorm(z2))*DtransY.T)^Delta)*(((1/sigma2)*dnorm(z2)*(1-pnorm(z1))*DtransY.C)^(1-Delta))
  p1 = pmax(tot,1e-100) 
...
# Same function as SimulationCI11 but only computing the simulation for the two-
# step estimator and oracle estimator.
SimulationCI11_SaraIlias_simplified = function(n, nsim, iseed, init.value.theta_1, init.value.theta_2) {
  sum = c()
  sum1 = c()
  sum2 = c()
  sum3 = c()
...
DataApplicationJPTA <- function(data, init.value.theta_1, init.value.theta_2) {
  ...
  parhat1 <- nloptr(x0=c(init),eval_f=LikI_noTransform,Y=Y,Delta=Delta,M=M,lb=c(rep(-Inf,totparl),1e-05,1e-5),ub=c(rep(Inf,totparl),Inf,Inf),
                    eval_g_ineq=NULL,opts = list(algorithm = "NLOPT_LN_BOBYQA","ftol_abs"=1.0e-30,"maxeval"=100000,"xtol_abs"=rep(1.0e-30)))$solution
  initd = c(parhat1,0)
  parhat_noTransform <- nloptr(x0=initd,eval_f=LikF_noTransform,Y=Y,Delta=Delta,M=M,lb=c(rep(-Inf,totparl),1e-05,1e-5,-1),ub=c(rep(Inf,totparl),Inf,Inf,1),
                               eval_g_ineq=NULL,opts = list(algorithm = "NLOPT_LN_BOBYQA","ftol_abs"=1.0e-30,"maxeval"=100000,"xtol_abs"=rep(1.0e-30)))$solution
