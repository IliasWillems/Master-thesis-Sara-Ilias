...
dat.sim.reg = function(n,par,iseed,Zbin,Wbin){
  ...
  Y = log(pmin(exp(T),exp(C),A)) # observed non-transformed time
  d1 = as.numeric(Y==T) # censoring indicator
  xi1 = ifelse(d1==1,0,as.numeric(Y==C))
...
LikFNT = function(par,Y,Delta,M){ 
  ...
  tot = (((1/sigma1)*dnorm(z1)*(1-pnorm(z2)))^Delta)*(((1/sigma2)*dnorm(z3)*(1-pnorm(z4)))^Xi)*((pbinorm(q1=-z1,q2=-z3,cov12=rho))^(1-(Delta+Xi))) # likelihood
  p1 = pmax(tot,1e-100)   
  Logn = sum(log(p1)); 
  return(-Logn)
}
...
LikFNTG1 = function(par,Y,Delta,Xi,M){ 
  M=as.matrix(M)
  k = ncol(M)-2 # remove Z and W
  l = 2*(k+1)
  v = k+3
...
LikFNTG2 = function(par,Y,Delta,Xi,M){ 
  M=as.matrix(M)
  k = ncol(M)-2
  l = 2*(k+1)
  v = k+3
