...
data.misspecified.skew <- function(n, par, iseed, Zbin, Wbin) {
  ...
  Mgen = matrix(c(x0,x1,Z,realV),ncol=parl,nrow=n)  # matrix containing all covariates
  T = Mgen%*%beta+Z_1 # model YJ transformed time with real covariates
  C = Mgen%*%eta+Z_2 # model YJ transformed censoring with real covariates
  ...
  Y = pmin(T,C) # observed YJ transformed time
  d1 = as.numeric(Y==T) # censoring indicator
  Y = IYJtrans(Y,theta) # observed time
  data = cbind(Y,d1,M,realV) # data consisting of observed time,
  # censoring indicator, all data and the control function
...
Misspecification.skew.sim = function(n, nsim, iseed, par, Zbin, Wbin) {
  ...
  par0 = c(par[[1]], par[[2]], c(-1, -1, theoretical.correlation, par[[3]][4]))
  par0m = matrix(par0,nsim,(totparl+4),byrow=TRUE)
